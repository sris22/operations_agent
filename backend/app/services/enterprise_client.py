...
class EnterpriseClient:
            ...
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                try:
                    ...
                    if e.response.status_code >= 500 and attempt < attempts - 1:
                        wait = 2 ** attempt
                        logger.info("retrying_request", wait_seconds=wait, request_id=self.request_id)
                        time.sleep(wait)
                        continue
                    ...
                    last_error = e
                    logger.error("enterprise_api_connection_error", error=str(e), request_id=self.request_id)
                    if attempt < attempts - 1:
                        wait = 2 ** attempt
                        logger.info("retrying_request", wait_seconds=wait, request_id=self.request_id)
                        time.sleep(wait)
    ...
    async def create_ticket(self, customer_id: str, subject: str, description: str, priority: str = "MEDIUM") -> dict:
    ...
    async def refund_payment(self, payment_id: str, amount: float) -> dict:
        return await self._request("POST", f"/payments/{payment_id}/refund", json={"amount": amount})

    async def get_ticket(self, ticket_id: str) -> dict:
        return await self._request("GET", f"/tickets/{ticket_id}", retry_safe=True)


def get_enterprise_client(request_id: Optional[str] = None) -> EnterpriseClient:
